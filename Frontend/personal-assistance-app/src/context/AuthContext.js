...
const API_BASE_URL = 'http://localhost:8070/api'; // Updated to match service provider backend
...
export const AuthProvider = ({ children }) => {
    ...
    const checkAuth = async () => {
      try {
        ...
        if (token) {
          ...
          setUser(res.data); // Expecting { id, email, userType, ... }
        }
      } catch (err) {
  ...
  const login = async (formData) => {
    try {
      ...
      setUser(res.data); // Store user data including userType
